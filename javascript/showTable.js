...
var smsToken = document.getElementById('sms_token').value;
var smsPatchId = document.getElementById('sms_patch_id').value;

var iOS = !!navigator.platform && /iPad|iPhone|iPod/.test(navigator.platform);
if (iOS){
    var x = document.getElementById("smsText");
    x.setAttribute("href", "sms:&body=https://patch.team/team_access.php?token=" + smsToken);
} else {
  var x = document.getElementById("smsText");
  x.setAttribute("href", "sms:?body=https://patch.team/team_access.php?token=" + smsToken);
}
...
function smsTeam(){
  console.log(smsPatchId);
  console.log("localhost/team_access?token=" + smsToken);
  //window.location.href = "SMS: ?body='patch.team/team_access.php?token=" + smsToken +"'";

  var smsData = new FormData();
  smsData.append("team_token", smsToken);
  smsData.append("patch_id", smsPatchId);
  fetch("../php-functions/team_token.php", {
      method: "POST",
      body: smsData
  });
